...
function ContactLink({ href, icon, title, detail }: { href: string; icon: IconName; title: string; detail: string }) {
  return (
    <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noopener noreferrer" : undefined}
      className="group flex items-center gap-4 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4 transition duration-300 hover:-translate-y-0.5 hover:border-cyan-300/20 hover:bg-white/[0.045]">
      <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[0.04] text-cyan-200 transition group-hover:border-cyan-300/30 group-hover:bg-cyan-300/10"><Icon name={icon} /></span>
      ...
      <Icon name="arrow" className="h-4 w-4 text-slate-700 transition group-hover:translate-x-1 group-hover:text-cyan-200" />
    </a>
  );
}
...
export default function ContactPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#06020f] px-5 pb-20 pt-28 text-white sm:px-8 lg:pt-32">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute -right-40 top-1/3 h-96 w-96 rounded-full bg-cyan-400/[0.08] blur-[110px]" />
      ...
      <div className="relative mx-auto max-w-6xl">
        <header className="mb-12 max-w-3xl lg:mb-16">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/[0.06] px-4 py-2 text-xs font-medium uppercase tracking-[0.2em] text-cyan-200">
          ...
          <h1 className="text-5xl font-black tracking-[-0.055em] sm:text-6xl lg:text-7xl">Let&apos;s build something <span className="bg-gradient-to-r from-violet-400 via-indigo-300 to-cyan-200 bg-clip-text text-transparent">worth remembering.</span></h1>
          ...
          <aside className="space-y-5">
            <section className="rounded-3xl border border-white/[0.08] bg-white/[0.025] p-6 backdrop-blur-xl sm:p-7">
              ...
              <div className="mt-5 flex items-center justify-between gap-3 rounded-2xl border border-white/[0.07] bg-black/20 p-4">
                <a href={`mailto:${EMAIL}`} className="min-w-0 truncate text-sm font-medium text-slate-200 hover:text-cyan-200">{EMAIL}</a>
                <button type="button" onClick={copyEmail} aria-label="Copy email address" className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-white/10 text-slate-400 transition hover:border-cyan-300/30 hover:bg-cyan-300/10 hover:text-cyan-200">
            ...
            <section className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5"><Icon name="spark" className="mb-4 h-5 w-5 text-cyan-200" /><p className="text-sm font-semibold">Bengaluru, India</p><p className="mt-1 text-xs leading-5 text-slate-600">Open to remote work</p></div>
            </section>
          </aside>

          <section className="relative overflow-hidden rounded-[2rem] border border-white/[0.09] bg-[#0d0718]/85 p-6 shadow-2xl shadow-violet-950/30 backdrop-blur-2xl sm:p-9">
            <div aria-hidden="true" className="absolute inset-x-12 top-0 h-px bg-gradient-to-r from-transparent via-cyan-200/70 to-transparent" />
            {success ? (
              <div className="flex min-h-[34rem] flex-col items-center justify-center text-center" role="status">
                ...
                <button type="button" onClick={() => setSuccess(false)} className="mt-8 inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-semibold transition hover:border-cyan-300/30 hover:bg-cyan-300/10">Send another message <Icon name="arrow" className="h-4 w-4" /></button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  <div><label htmlFor="name" className="mb-2 block text-sm font-medium text-slate-300">Your name</label><input id="name" name="name" autoComplete="name" required maxLength={100} value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Jane Smith" className="w-full rounded-xl border border-white/[0.09] bg-white/[0.035] px-4 py-3.5 text-sm outline-none transition placeholder:text-slate-700 focus:border-cyan-300/40 focus:bg-white/[0.055] focus:ring-4 focus:ring-cyan-300/[0.06]" /></div>
                  <div><label htmlFor="email" className="mb-2 block text-sm font-medium text-slate-300">Email address</label><input id="email" name="email" type="email" autoComplete="email" required maxLength={254} value={form.email} onChange={(event) => update("email", event.target.value)} placeholder="jane@company.com" className="w-full rounded-xl border border-white/[0.09] bg-white/[0.035] px-4 py-3.5 text-sm outline-none transition placeholder:text-slate-700 focus:border-cyan-300/40 focus:bg-white/[0.055] focus:ring-4 focus:ring-cyan-300/[0.06]" /></div>
                </div>
                <div><label htmlFor="projectType" className="mb-2 block text-sm font-medium text-slate-300">What can I help with?</label><div className="relative"><select id="projectType" value={form.projectType} onChange={(event) => update("projectType", event.target.value)} className="w-full appearance-none rounded-xl border border-white/[0.09] bg-[#110a1d] px-4 py-3.5 text-sm text-slate-200 outline-none transition focus:border-cyan-300/40 focus:ring-4 focus:ring-cyan-300/[0.06]">{projectTypes.map((type) => <option key={type}>{type}</option>)}</select><span aria-hidden="true" className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-xs text-slate-600">v</span></div></div>
                <div><div className="mb-2 flex items-center justify-between gap-4"><label htmlFor="message" className="text-sm font-medium text-slate-300">Project details</label><span className={`text-xs tabular-nums ${form.message.length > MAX_MESSAGE_LENGTH * 0.9 ? "text-amber-300" : "text-slate-700"}`}>{form.message.length}/{MAX_MESSAGE_LENGTH}</span></div><textarea id="message" name="message" required minLength={10} maxLength={MAX_MESSAGE_LENGTH} value={form.message} onChange={(event) => update("message", event.target.value)} placeholder="What are you building, where are you stuck, and what would a great outcome look like?" className="min-h-40 w-full resize-y rounded-xl border border-white/[0.09] bg-white/[0.035] px-4 py-3.5 text-sm leading-6 outline-none transition placeholder:text-slate-700 focus:border-cyan-300/40 focus:bg-white/[0.055] focus:ring-4 focus:ring-cyan-300/[0.06]" /></div>
                <button type="submit" disabled={loading} className="group flex w-full items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-violet-600 via-indigo-500 to-cyan-500 px-5 py-4 text-sm font-bold shadow-lg shadow-violet-950/30 transition duration-300 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-violet-900/30 disabled:cursor-wait disabled:opacity-60 disabled:hover:translate-y-0">
